...
class Blockchain:
    ...
    def is_chain_valid(self, chain):
        ...
        while block_index < len(chain):
            ...
            if block['prevoius_hash'] != self.hash(previous_block):
                return False
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, reciever = 'xyz', amount = 100)
...
def add_transaction():
    ...
    if not all (key in json for key in transaction_keys):
        return 'Elements of the transaction are missing', 400
    else:
...
def connect_node():
    ...
    response = {'message' : 'Total nodes are ': list(blockchain.nodes)}
    return jsonify(response), 201
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'The node has different chain so was replaced'}
    else:
